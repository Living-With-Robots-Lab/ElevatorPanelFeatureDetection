...
labels_correct, buttons_correct, total_labels, total_buttons = 0, 0, 0, 0
bbox_coords = []
...
class ElevatorDetectorLazyConf(nn.Module):
    ...
    def run(
        ...
        """
        assert len(imgs) == 1
        global tot_missed_found
        global temp_missed
        global last_checked
        global total_labels
        global total_buttons
        global labels_correct
        global buttons_correct
        global bbox_coords
        # save original image height and width
        height, width = imgs[0]["height"], imgs[0]["width"]
        original_img = Image.open(imgs[0]["img_path"])
        ...
        with torch.no_grad():
            ...
            detections = outputs[0]["instances"].to(self.device)
            #print(detections)
            detections_instances = outputs[0]["instances"].to(self.device)
            detections_count = len(detections_instances.get_fields()["pred_boxes"])
            ...
            if self.recovery_weights:
                ...
                recovered_instances = outputs[0]["instances"].to(self.device)
                print("Recovered Instances")
                print(recovered_instances)
                recovery_count = len(recovered_instances.get_fields()["pred_boxes"])
                pred_labels = recovered_instances[recovered_instances.pred_classes == 0].pred_boxes
                pred_btns = recovered_instances[recovered_instances.pred_classes == 1].pred_boxes

                if len(pred_labels) > 0:
                    bbox_coords.append(["l", recovered_instances[recovered_instances.pred_classes == 0].pred_boxes.to(self.device).tensor.tolist()[0]])
                if len(pred_btns) > 0:
                    bbox_coords.append(["b", recovered_instances[recovered_instances.pred_classes == 1].pred_boxes.to(self.device).tensor.tolist()[0]])

                #print(f"Num Recovered: {recovery_count}")
                #tot_missed_found += recovery_count
                #print(f"Total Recovered: {tot_missed_found}")
                # merge both detections into one output
                detections = Instances.cat([detections, recovered_instances])
                visualizer = Visualizer(
                    np.asarray(original_img),
            ...
            '''
            if tot_missed > 0:
                tot_stats = tot_missed_found/tot_missed
                print(f"\n\nRunning Numbers: {tot_stats}")
            '''
...
def test_pipeline_acc(dataset):
    ...
    runtime = 0.0
    tot_missed = 0
    total_buttons, total_labels = 0, 0
    test_iou = 0.4
    ########
...
#            continue
        local_missed, local_correct, local_total, local_runtime = 0, 0, 0, 0.0
        ...
        print(labels.items())
        last_checked = False
        local_labels_correct = 0
        local_buttons_correct = 0
        local_total_labels = 0
        local_total_buttons = 0
...
#                continue
            ...
            label_bbox = outputs["target_label"].pred_boxes[0].to("cpu")
            label_bbox = label_bbox.tensor.tolist()[0]
            ...
            label_iou = calc_iou(label_bbox, gt_bbox)
            ...
            for pair in bbox_coords:
                print(pair)
                if pair[0] == "l" and calc_iou(label_bbox, pair[1]) > 0.75:
                    if label_iou > test_iou:    
                        local_labels_correct += 1
                elif pair[0] == "b" and calc_iou(btn_bbox, pair[1]) > 0.75:
                    if iou > test_iou:
                        local_buttons_correct += 1
        ...
        buttons_correct += local_buttons_correct
        print(labels_correct)
        print(buttons_correct)
        correct += local_correct
        total += local_total
        runtime += local_runtime
        #tot_missed += total - temp_missed
    ...
    #labels_info = labels_correct/total_labels
    #buttons_info = buttons_correct/total_buttons
    #both_info = (labels_correct + buttons_correct)/(total_labels + total_buttons)
    log_every_n(logging.INFO, f"recovery label stats: {labels_correct}")
    log_every_n(logging.INFO, f"recovery button stats: {buttons_correct}")
    #log_every_n(logging.INFO, f"total recovery stats: {labels_correct + buttons_correct}/{total_labels + total_buttons} = {both_info}")
...
def test_button_accuracy(
    ...
    # Register mixed datasets
    for spl, im_paths in zip(["train", "val", "test"], datasets):
        if not f"{dataset_name}_{spl}" in DatasetCatalog.list():    
            DatasetCatalog.register(
                f"{dataset_name}_{spl}",
                lambda im_paths=im_paths: register_dataset(im_paths),
            )
...
if __name__ == "__main__":
    ...
    #recovery_acc = tot_missed_found / tot_missed
    #print(f"Complete Recovery Results: {tot_missed_found}/{tot_missed} = {recovery_acc}")
    # test_vit_segmentation("mixed")
    # test_resnet_segmentation(dataset_name="mixed")
    for iou in [0.5, 0.6, 0.7, 0.8, 0.9]:
        with open("accuracy_results_mixed_IROS_changed_params.txt", "a") as f:
            f.writelines(
